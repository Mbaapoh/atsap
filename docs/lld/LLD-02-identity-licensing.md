...
| ~~`identity-bootstrap`~~ | ~~Dev-token minting for the rig, UAT script updates~~ | **Closed 2026-09-08 as unnecessary** (`auth-cutover-connectrpc` design D4). The rig obtains a real token through the real `AuthenticateUser` RPC, seeding a fixture operator directly — which is a better test than a minted one, since a dev token proves the rig can reach the API rather than that a caller can. A second token-minting path would also be a second thing to keep secure, and a second thing that can be left enabled in production by accident |
