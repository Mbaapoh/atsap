...
- [ ] 4.1 Add `ReleaseCapacity(ctx context.Context, channels int) error` to `telephony/ports.LicenseManager`; verify `go build ./...` fails until every implementation is updated, then passes (design D1)
- [ ] 4.2 Call `ReleaseCapacity` once in `telephony/application/service.go`'s `finalizeTermination`, only for calls that left Screening permitted; verify unit tests cover normal hangup, setup failure, screened-out (no release) and double termination (no second release) (spec `telephony-core/call-lifecycle`, design D3)
